...
from src.simulation_without_package.set_up_control import set_up_control
...
def set_up_model(self,image_path):
	...
	# set up control u
	test = 0 # 0: zero control 1: sudden high temperature
	...
	def animate(k):
		...
		plt.pcolormesh(C, cmap=plt.cm.jet, vmin=0,vmax=1)
		plt.colorbar()
		if k%5 == 0:
	...
	if noiselevel > 0:
		...
		noise = distributions.MultivariateNormal(torch.zeros(l),gamma).sample((Nt+1,))
		noise = torch.t(noise)
